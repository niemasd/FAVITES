...
CITATION_DUALBIRTH = 'Moshiri, N. and Mirarab, S. (2017). "A Two-State Model of Tree Evolution and its Applications to Alu Retrotransposition". JOURNAL INFORMATION TBD.'
...
def tn_favites2gexf(cn,tn):
    ...
    for node in nodes:
        ...
        if len(times) == 0: # uninfected nodes
            out += '          <attvalue for="infected" value="false" start="0.0" end="%s" />\n' % str(time+1)
        elif times[0] == 0: # seed node
            out += '          <attvalue for="infected" value="true" start="0.0" end="%s" />\n' % str(time+1)
        else: # regular infected nodes
            out += '          <attvalue for="infected" value="false" start="0.0" end="%s" />\n' % str(times[0])
            out += '          <attvalue for="infected" value="true" start="%s" end="%s" />\n' % (str(times[0]),str(time+1))
        out += '        </attvalues>\n'
        out += '      </node>\n'
    out += '    </nodes>\n'
    out += '    <edges>\n'
    ...
    for u,v in edges:
        ...
        for i in range(len(times)-1):
            out += '          <attvalue for="transmission" value="%s" start="%s" end="%s" />\n' % (str(i != 0).lower(),str(times[i]),str(times[i+1]))
        out += '        </attvalues>\n'
        out += '      </edge>\n'
    out += '    </edges>\n'
    out += '  </graph>\n'
