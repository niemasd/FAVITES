...
a rate that is sampled from a user-parameterized Normal (Gaussian) distribution.
'''
from TreeUnit import TreeUnit
import FAVITES_GlobalContext as GC

class TreeUnit_Normal(TreeUnit):
    def cite():
    ...
    def time_to_mutation_rate(tree):
        ...
        for node in t.traverse_preorder():
            if node.edge_length is not None:
                node.edge_length *= normal(loc=GC.tree_rate_mean,scale=GC.tree_rate_stdev)
        return str(t)
